...
for i in $(seq 1 $N_JOBS); do
    ...
    sbatch --job-name="$JOB_NAME" --export=ALL,L=$L,P_RANGE="$P_RANGE",P_FIXED_NAME="$P_FIXED_NAME",P_FIXED_VALUE=$P_FIXED_VALUE,ANCILLA=$ANCILLA,MAXDIM=$MAXDIM,THRESHOLD=$THRESHOLD,N_CHUNK_REALIZATIONS=$N_CHUNK_REALIZATIONS,OUTPUT_DIR="$OUTPUT_DIR",JOB_NAME="$JOB_NAME",JOB_COUNTER=$i --mem=$MEMORY $SLURM_SCRIPT
done